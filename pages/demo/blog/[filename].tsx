...
const BlogPage = (props: { [key: string]: any }) => {
  const { data } = useTina({
    query: props.query,
...
export const getStaticPaths = async () => {
  ...
  const paths = postsListData.data.postConnection.edges
    .filter((post) => post?.node?._sys?.filename)
    .map((post) => ({
...
const PageSection = (props: { [key: string]: any }) => {
  return (
    <>
      <h2>{props.heading}</h2>
      <p>{props.content}</p>
